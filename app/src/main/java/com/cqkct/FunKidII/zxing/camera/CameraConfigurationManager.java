...
package com.cqkct.FunKidII.zxing.camera;
...
//	private static final int MAX_PREVIEW_PIXELS = 800 * 800; // large/HD screen
	...
	void initFromCameraParameters(Camera camera) {
		...
		// and reverse them:
		if (width < height) {
			L.i(TAG, "Display reports portrait orientation; assuming this is incorrect");
			int temp = width;
			width = height;
			height = temp;
		}
	...
	void setDesiredCameraParameters(Camera camera) {
		...
		if (parameters == null) {
			L.w(TAG, "Device error: no camera parameters are available. Proceeding without configuration.");
			return;
		}
	...
	private static Point findBestPreviewSizeValue(Camera.Parameters parameters, Point screenResolution, boolean portrait) {
		...
		for (Camera.Size supportedPreviewSize : parameters.getSupportedPreviewSizes()) {
			L.d(TAG, "findBestPreviewSizeValue: supportedPreviewSize: " + supportedPreviewSize.width + "x" + supportedPreviewSize.height);
	...
	private static String findSettableValue(Collection<String> supportedValues, String... desiredValues) {
		L.i(TAG, "Supported values: " + supportedValues);
		String result = null;
