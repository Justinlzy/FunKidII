...
public class WheelView extends View implements GestureDetector.OnGestureListener {
    ...
    protected void init(AttributeSet attrs) {
        ...
        mMarkColor = 0xFF7D7D7D
        ;
        mCursorSize = density * 15;
        mCenterTextSize = density * 20;
        mNormalTextSize = density * 15;
        mBottomSpace = density * 4;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = start; i < end; i++) {
            float tempDis = mIntervalDis / 10f;
            ...
            for (int offset = -5; offset < 5; offset++) {
                ...
                if (offset == 0) {
                    // center mark
                    mMarkPaint.setStrokeWidth(mCenterMarkWidth);
//					canvas.drawLine(ox, height * 0.1f, ox, height * 0.2f , mMarkPaint);
                    canvas.drawLine(ox, height * 0.8f, ox, height * 0.7f, mMarkPaint);
                    canvas.drawLine(ox, height * 0.2f, ox, height * 0.3f, mMarkPaint);
                } else {
                    // other small mark
            ...
            if (mMarkCount > 0 && i >= 0 && i < mMarkCount) {
                ...
                if (mCenterIndex == i) {
                    mMarkTextPaint.setColor(0xff11CBD8);
                    mMarkTextPaint.setTextSize(mCenterTextSize);
                    if (!TextUtils.isEmpty(mAdditionCenterMark)) {
                        float off = mAdditionCenterMarkWidth / 2f;
                        float tsize = mMarkTextPaint.measureText(temp, 0, temp.length());
                        canvas.drawText(temp, 0, temp.length(), x - off, height * 0.55f, mMarkTextPaint);
                        mMarkTextPaint.setTextSize(mNormalTextSize);
                        canvas.drawText(mAdditionCenterMark, x + tsize / 2f, height * 0.55f, mMarkTextPaint);
                    } else {
                        canvas.drawText(temp, 0, temp.length(), x, height * 0.55f, mMarkTextPaint);
                    }
                } else {
