...
public class QRCodeUtils {
    ...
    public static Bitmap createCode(Context context, String content, Bitmap logo) throws WriterException {
        ...
        BitMatrix matrix = writer.encode(content, BarcodeFormat.QR_CODE,
                (int) DensityUtils.dp2px(context, 300),
                (int) DensityUtils.dp2px(context, 300), hst);
        // 矩阵高度
    ...
    public static Bitmap createCode(Context context, String content) throws WriterException {
        return createCode(context, content, 250f, 250f);
    }

    public static Bitmap createCode(Context context, String content, float widthDp, float heightDp) throws WriterException {
        return createCode(context, content, DensityUtils.dp2px(context, widthDp), DensityUtils.dp2px(context, heightDp));
    }

    public static Bitmap createCode(Context context, String content, int widthPx, int heightPx) throws WriterException {
        //生成二维矩阵,编码时指定大小,不要生成了图片以后再进行缩放,这样会模糊导致识别失败
        BitMatrix matrix = new MultiFormatWriter().encode(content, BarcodeFormat.QR_CODE, widthPx, heightPx);
        int width = matrix.getWidth();
        int height = matrix.getHeight();
