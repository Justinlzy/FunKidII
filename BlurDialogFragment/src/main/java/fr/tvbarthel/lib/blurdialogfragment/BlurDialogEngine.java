...
public class BlurDialogEngine {
     ...
     * 是否始终需要模糊状态栏
     */
    private boolean mAlwaysBlurStatusBar;
    ...
    /**
     * Enable / disable blurred status bar.
     * <p/>
     * When enabled, the status bar is blurred in addition of the content.
     *
     * @param blur true to blur the status bar.
     */
    public void setAlwaysBlurStatusBar(boolean blur) {
        mAlwaysBlurStatusBar = blur;
    }
    ...
    private void blur(Bitmap bkg, View view) {
        ...
        final int topOffset = statusBarHeight + actionBarHeight + mTitleBarHeight;
        // evaluate bottom or right offset due to navigation bar.
        int bottomOffset = 0;
        int rightOffset = 0;
        final int navBarSize = getNavigationBarOffset();

        View contentView = mHoldingActivity.findViewById(android.R.id.content);
